...
public interface InscripcionRep extends PagingAndSortingRepository<Inscripcion, Integer>{
	...
	@Query("SELECT i FROM Inscripcion i where i.tempProfesor = :tempProfesor AND i.tempGrupo = :tempGrupo")
	Inscripcion findByListProfesor(
			@Param("tempProfesor") String tempProfesor,
			@Param("tempGrupo") String tempGrupo
	);
