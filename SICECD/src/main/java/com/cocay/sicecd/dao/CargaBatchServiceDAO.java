...
//		int total = ((Number) queryMax.getSingleResult()).intValue();
	...
	public Integer totalLstErrores(String search) throws Exception {
		Query query = em.createNativeQuery("SELECT count(1) FROM errores WHERE estado = 1");
		if(search != null && !search.equals("")) {
	...
	public void lstLimpiar() throws Exception {
		Query query = em.createNativeQuery("UPDATE errores SET estado = 2 WHERE estado = 1");
		query.executeUpdate();
